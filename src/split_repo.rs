...
impl SplitRepo {
    pub async fn get_all(db: DB) -> Result<Vec<Split>, Error> {
        let split_dtos: Vec<SplitDTO> = sqlx::query_as("SELECT id, description, usernames FROM splits")
            .fetch_all(db.get_pool())
            .await?;

        Ok(split_dtos.into_iter().map(|dto: SplitDTO| dto.into()).collect())
    }

    pub async fn get_by_id(db: DB, id: String) -> Result<Split, Error> {
        let dto: SplitDTO = sqlx::query_as("SELECT id, description, usernames FROM splits WHERE id = ?")
            .bind(id)
    ...
    pub async fn create(db: DB, split: Split) -> Result<(), Error> {
